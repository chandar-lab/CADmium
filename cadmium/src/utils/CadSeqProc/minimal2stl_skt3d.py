...
def main():
    ...
    futures = [
        executor.submit(
            process_one, os.path.join(args.input_dir, json_uid+ '_' + args.level, json_uid.split('/')[1] +  '_' + args.level +  ".json"), args
        )
...
def process_one(file_path,args):
    ...
    try:
        ...
        if file_type == "json":
            ...
            json_data = remove_nulls(json_data)
            cad_seq = CADSequence.minimal_json_to_seq(
