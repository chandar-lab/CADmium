...
def main():
    ...
    parser.add_argument("--ckpt",help="model checkpoint",required=True)
    parser.add_argument("--verbose",action='store_true')

    args=parser.parse_args()
    jsons_dir = '/home/mila/b/baldelld/scratch/LLM4CAD/cadmium/data/generated_jsons'
    eval_dir = '/home/mila/b/baldelld/scratch/LLM4CAD/cadmium/data/generated_evals'
    input_path = os.path.join(jsons_dir, args.ckpt)
    output_dir = os.path.join(eval_dir, args.ckpt)
    output_dir=create_path_with_time(output_dir)
    ...
    for folder in folders:
        paths = os.listdir(os.path.join(input_path, folder))
        for path in paths:
            uids.append(path.split('_')[0][:4] + '/' + path.split('_')[0])
    output_path = os.path.join(output_dir)
    ensure_dir(output_path)
    generate_analysis_report(uids=uids, input_path = input_path, output_path=output_path,
